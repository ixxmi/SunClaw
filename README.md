...
- 多渠道接入：支持企业微信、飞书、Telegram、钉钉、QQ、Slack、Discord、Google Chat、Teams、iMessage 等。
...
对于绑定到 ACP thread 的长任务会话，还支持：

- `/interrupt`、`/stop`、`/cancel`
  中断当前正在执行的任务
- `/resume <内容>`、`/continue <内容>`
  按补充说明继续当前 ACP 会话
- 运行中直接发送一条新消息
  会自动中断当前任务，并按最新消息继续
...
- `channels`
  配置企业微信、飞书、Telegram、WhatsApp、iMessage 等接入信息
