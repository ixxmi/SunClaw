...
type Message struct {
	...
	ReasoningContent string     `json:"reasoning_content,omitempty"` // deepseek/qwen 思考内容
	Images           []string   `json:"images,omitempty"`            // Image URLs or Base64
	ToolCallID       string     `json:"tool_call_id,omitempty"`      // For tool role
	ToolName         string     `json:"tool_name,omitempty"`         // For tool role
...
type Response struct {
	Content      string     `json:"content"`
	Reasoning    string     `json:"reasoning,omitempty"` // 思考过程（reasoning_content / reasoning）
