...
type AgentState struct {
	// SystemPrompt 存放第 4 层的 agent core：
	// - 主 agent：自定义 prompt，或回退到系统通用认知
	// - 子 agent：目标 agent 的 core prompt
	// 最终完整 system prompt 由运行时统一装配。
	SystemPrompt string
	Model        string
	Provider     string
	AgentID      string // 当前 Agent ID，用于工具上下文传递
	// BootstrapOwnerID 标识当前运行应使用哪一个主 agent 的认知文件。
	// 主 agent 运行时通常等于 AgentID；子 agent 运行时通常等于目标 agent。
	BootstrapOwnerID string
	ThinkingLevel    string // off, minimal, low, medium, high, xhigh
	Tools            []Tool
	// SpawnableAgentCatalog 仅用于主 agent，表示可派生 agent 的动态目录。
	SpawnableAgentCatalog string
	// SubagentDescriptor 仅用于子 agent，表示这次运行时的分身职责和任务约束。
	SubagentDescriptor string
	// IsSubagent 标记当前 state 是否用于子 agent。
	IsSubagent    bool
	...
	ToolFailCount     int // 工具调用失败次数
	ConsecutiveErrors int // 连续错误次数（成功后归零）
	ReflectionCount   int // 反思触发次数

	// PendingSubagents 记录已派发但尚未返回结果的子 agent 数量。
	// 使用 atomic 操作，子 agent goroutine 完成时从外部线程递减，runLoop 在主线程读取。
	PendingSubagents int64
...
func (s *AgentState) AddPendingSubagent() {
	atomic.AddInt64(&s.PendingSubagents, 1)
}

// RemovePendingSubagent 子 agent 完成时递减（线程安全）
func (s *AgentState) RemovePendingSubagent() {
	atomic.AddInt64(&s.PendingSubagents, -1)
}

// HasPendingSubagents 是否还有未完成的子 agent（线程安全）
func (s *AgentState) HasPendingSubagents() bool {
	return atomic.LoadInt64(&s.PendingSubagents) > 0
}
...
type LoopConfig struct {
	...
	// 反思机制配置（参考 picoclaw）
	// MaxConsecutiveErrors：连续工具失败多少次触发反思（默认 3）
	MaxConsecutiveErrors int
	// MaxToolFailures：总工具失败次数上限，超出直接退出（默认 10）
	MaxToolFailures int
	// EnableReflection：是否开启反思机制（默认 true）
...
func (s *AgentState) Clone() *AgentState {
	...
	return &AgentState{
		...
		SpawnableAgentCatalog: s.SpawnableAgentCatalog,
		SubagentDescriptor:    s.SubagentDescriptor,
