...
func TestAssemblePrompt_TemplateCognitionIsInjectedAndBootstrapFollowsAgents(t *testing.T) {
	...
	for _, marker := range []string{"### AGENTS.md", "### BOOTSTRAP.md", "### IDENTITY.md", "### SOUL.md", "### USER.md", "custom core"} {
	...
	if !(agentsIdx < bootstrapIdx && bootstrapIdx < identityIdx && identityIdx < soulIdx && soulIdx < userIdx && userIdx < coreIdx) {
		t.Fatalf("expected AGENTS -> BOOTSTRAP -> IDENTITY -> SOUL -> USER -> system_prompt order, got %q", got)
	}
}
...
func TestAssemblePrompt_DoesNotAppendBootstrapWhenOnlyAgentsOrSoulRemainTemplate(t *testing.T) {
	...
	for _, marker := range []string{"custom identity", "custom user", "### AGENTS.md", "### SOUL.md"} {
		if !strings.Contains(got, marker) {
